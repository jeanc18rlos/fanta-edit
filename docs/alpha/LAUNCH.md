...
changes. The latest local native build passed in 10m27s with two targeted memory fixes. Repeated project open/close, chooser cancellation, new-design save, and native alert checks passed; independent scans no longer show the traced callback/list retain cycles. Other scanner findings remain unresolved. Earlier builds include the 5m30s inspector candidate and 7m19s rendering
