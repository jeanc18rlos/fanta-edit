...
- Desktop account/provider checks, 37 native generation tests, 36 document tests, 34 inspector tests, and the focused Sidebar regressions passed. The full Sidebar suite has 137 passes and six existing failures; it is not fully green. Generation now accepts backend string seeds and sends integer segmentation coordinates; sidebar selection follows the same item through list updates. Four selection tests passed 20 scheduler iterations each.
...
Current backend `a2bb2bb` is live as `dpl_7YZNhCekZv7wGpXgZFrivLsB2FTT`. All 396 tests, type checking, GitHub CI, Docker/GPU checks, and the production build pass. REST and MCP production catalogs exclude mock models; three direct mock requests returned 400 with no credit debit. A fresh default Sonnet stream then passed with one credit debited (388 → 387), usage recorded, and the temporary key revoked and rejected. These API checks do not establish final native sign-in, real media generation, all models, or customer payment.

Path Selection now edits existing vector anchors, handles, and segment endpoints with undo/redo. All 336 document, 258 editing-tool, and 15 native toolbar tests pass; the new toolbar regression passes 20 scheduler iterations. Shared curve bounds include control points, which may enlarge curved selection/layout extents. Final installer visual validation remains pending; Scale and Text on Path remain placeholders.
