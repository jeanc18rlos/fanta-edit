...
async fn show_store(
    ...
    let user_id = fetch_backend_user_id(&client, &token, &server_url).await?;
    let revenuecat = RevenueCat::get_or_configure(REVENUECAT_PUBLIC_API_KEY, &user_id)
        .await
...
async fn fetch_backend_user_id(client: &Client, token: &str, server_url: &str) -> Result<String> {
