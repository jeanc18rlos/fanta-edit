...
async fn open_workspaces(
    ...
    if grouped_locations.is_empty() {
        // Nothing to open: an empty center pane renders the welcome page. Same
        // shape as `restore_or_create_workspace` - no onboarding tour, and no
        // untitled buffer, which is not a thing a design app can open.
