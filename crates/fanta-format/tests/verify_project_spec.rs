...
/// sidecar (v2: `pages/<page-root-id>/page.ids.json`, not a per-node file).
#[test]
fn verify_ids_survive_and_paths_are_derivable() {
    ...
    // child are both recorded in the page's sidecar.
    assert!(
        dir.path()
            .join(format!("pages/{}/page.fnx", f.page))
            .is_file(),
        "page source missing"
    );
    let page_sc = sidecar(&dir.path().join(format!("pages/{}/page.ids.json", f.page)));
    assert!(has(&page_sc, &bare(f.page)), "page root id in sidecar");
    assert!(has(&page_sc, &bare(f.child)), "child id in sidecar");
    // The component master files under components/<cid>/, not the page.
    assert!(
        dir.path()
    ...
    let comp_sc = sidecar(
        &dir.path()
            .join(format!("components/{}/master.ids.json", f.comp_id)),
    );
    assert!(
