//! Frame creation tool — drag-draws a FRAME (clipping group with a white
//! background), the "Default" option of the toolbar's frame slot. Identical
//! interaction model to the rect tool; only the committed node differs.
//!
//! ## Behavior
//!
            ...
            PointerEvent::Release {
                ...
                if rect.width() > 0.0 && rect.height() > 0.0 {
                    ...
                    if let Err(e) = ctx.doc.apply(Operation::create_node(node)) {
                        tracing::warn!(target: "fanta-tools.frame", "create failed: {e}");
                    } else {
