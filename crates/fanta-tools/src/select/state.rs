...
pub(super) struct ResizingState {
    ...
    /// scale — text, so its glyphs reflow instead of stretching like a scaled
    /// vector. Holds the node's variant data at press time: the preview rewrites
    /// the box `local_size` each frame, and commit records a
