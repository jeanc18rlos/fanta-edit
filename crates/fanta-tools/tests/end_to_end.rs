...
fn flow_rect_then_select_then_move() {
    ...
    // Debug: inspect the created shape's fills/color to diagnose "land without color"
    if let Some(node) = ctx.doc.scene.get(id) {
        if let fanta_doc::NodeData::Vector(v) = &node.data {
            eprintln!(
                "DEBUG created rect: fills.len={}, first_fill={:?}",
                v.fills.len(),
                v.fills.first()
            );
        }
