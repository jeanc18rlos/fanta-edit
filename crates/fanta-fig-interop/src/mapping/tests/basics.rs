...
#[test]
fn organizational_group_keeps_a_resizable_non_clipping_box() {
    let fig = doc_from(vec![
        o(
            "NodeChange",
            vec![
        ...
        o(
            ...
            vec![
                ("guid", guid(0, 2)),
                ("parentIndex", parent_index(0, 1)),
                ("type", KiwiValue::Enum("GROUP".into())),
                ("name", KiwiValue::String("Group".to_owned())),
                ("size", vector(180.0, 90.0)),
            ],
        ),
    ]);

    let (doc, _, _) = fig_to_doc(&fig).unwrap();
    let group = group_named(&doc, "Group");
    assert_eq!(group.local_size, Some([180.0, 90.0]));
    assert_eq!(group.clip_size, None);
    assert!(!group.is_frame_surface());
}
    ...
    let make = |valign: &str| {
        let fig = doc_from(vec![o(
            ...
            vec![
                ...
                ("textAlignVertical", KiwiValue::Enum(valign.into())),
            ],
        )]);
        let (doc, _, _) = fig_to_doc(&fig).unwrap();
...
fn invisible_and_gradient_paints_are_not_taken_as_solid_fill() {
    ...
    let gradient = o(
        "Paint",
        vec![("type", KiwiValue::Enum("GRADIENT_LINEAR".into()))],
    );
    let fig = doc_from(vec![o(
        "NodeChange",
        vec![
    ...
    let fig = doc_from(vec![
        o(
            ...
            vec![
                ...
                ("size", vector(10.0, 10.0)),
                ("blendMode", KiwiValue::Enum("NORMAL".into())),
            ],
        ),
        o(
            "NodeChange",
