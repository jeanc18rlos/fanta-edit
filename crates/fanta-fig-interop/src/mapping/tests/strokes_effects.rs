...
fn stroke_imports_paint_weight_and_align() {
    let rect = o(
        ...
        vec![
            ...
            ("strokeAlign", KiwiValue::Enum("INSIDE".to_owned())),
        ],
    );
...
fn node_opacity_blend_and_drop_shadow_import() {
    ...
    let rect = o(
        ...
        vec![
            ...
            ("blendMode", KiwiValue::Enum("MULTIPLY".to_owned())),
...
fn layer_and_background_blur_effects_import() {
    ...
    let layer_blur = o(
        "Effect",
        vec![
            ("type", KiwiValue::Enum("FOREGROUND_BLUR".to_owned())),
    ...
    let bg_blur = o(
        "Effect",
        vec![
            ("type", KiwiValue::Enum("BACKGROUND_BLUR".to_owned())),
    ...
    let hidden_blur = o(
        "Effect",
        vec![
            ("type", KiwiValue::Enum("LAYER_BLUR".to_owned())),
...
fn invisible_paint_is_skipped() {
    ...
    if let KiwiValue::Object { fields, .. } = &mut hidden {
        fields.insert("visible".to_owned(), KiwiValue::Bool(false));
    }
    let rect = o(
        "NodeChange",
        vec![
