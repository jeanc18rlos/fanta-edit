...
pub(crate) fn build_vector(
    ...
    if from_vector_network {
        if let Some(vd) = change.get("vectorData") {
            if let Some(blob) = vd.get("vectorNetworkBlob") {
                if let Some(idx) = blob.as_f64() {
                    node.meta["vector_network_blob"] = serde_json::json!(idx);
                }
            }
        }
    }
