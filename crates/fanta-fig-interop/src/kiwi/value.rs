...
///
/// Object keys and enum names are owned `String`s (the reference borrows
/// `&str` from the schema for zero-copy; we trade that for a value that does
/// not carry the schema's lifetime, which keeps the `.fig` mapping layer and
/// its tests far simpler — Figma schemas are read once per import, so the
...
pub enum KiwiValue {
    ...
    Object {
        type_name: String,
        fields: HashMap<String, KiwiValue>,
    },
}
...
impl KiwiValue {
    ...
    pub fn as_str(&self) -> Option<&str> {
        match self {
            KiwiValue::String(s) | KiwiValue::Enum(s) => Some(s),
            _ => None,
        }
    }
            ...
            idx if idx >= 0 => {
                ...
                match def.kind {
                    ...
                    DefKind::Struct => {
                        ...
                        for f in &def.fields {
                            fields.insert(f.name.clone(), Self::decode_field(schema, f, r)?);
                        }
                        KiwiValue::Object {
                    ...
                    DefKind::Message => {
                        ...
                        loop {
                            ...
                            match def.field_by_value(id) {
                                Some(f) => {
                                    fields
                                        .insert(f.name.clone(), Self::decode_field(schema, f, r)?);
                                }
...
mod tests {
    ...
    fn enum_round_trips_via_value() {
        ...
        assert_eq!(v, KiwiValue::Enum("ELLIPSE".to_owned()));
    ...
    fn message_with_all_fields_round_trips() {
        ...
        let v = obj(
            ...
            vec![
                ...
                ("shape", KiwiValue::Enum("RECT".to_owned())),
                (
                    "size",
                    obj(
                        "Vec2",
