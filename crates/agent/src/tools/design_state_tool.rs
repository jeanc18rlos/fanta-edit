...
/// so list with a small `depth` and go deeper by id.
...
impl AgentTool for DesignStateTool {
    ...
    fn run(
        ...
        cx.spawn(async move |cx| {
            ...
            let value = cx
                .update(|cx| {
                    ...
                    if let Some([width, height]) = input.empty_space {
                        ...
                        let listing = input.nodes.is_none();
                        surface.get_nodes(
                            NodeQuery {
            ...
            check_response_size(&text).map_err(tool_content_err)?;
            Ok(text.into())
        })
    }
}
...
/// `batch_get` twin enforces.
fn check_response_size(text: &str) -> Result<()> {
    if text.len() > design_surface::MAX_JSON_RESPONSE_BYTES {
        anyhow::bail!(
            "this result would be {} bytes, over the {}-byte response cap. Narrow it: lower \
             `depth` (try 1), set `include_geometry` to false, or pass the `nodes` ids you need.",
            text.len(),
...
mod tests {
    ...
    fn oversized_results_are_refused_with_a_narrowing_hint() {
        assert!(check_response_size("{}").is_ok());
        let error = check_response_size(&"n".repeat(design_surface::MAX_JSON_RESPONSE_BYTES + 1))
