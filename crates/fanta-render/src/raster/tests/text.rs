...
#[test]
fn first_baseline_tracks_vertical_alignment_in_the_text_box() {
    use fanta_doc::{TextNode, VAlign};

    let mut text = TextNode::new("Baseline", 160.0, 100.0);
    text.style.size_px = 20.0;
    text.vertical_align = VAlign::Top;
    let top = text_first_baseline(&text);
    text.vertical_align = VAlign::Center;
    let center = text_first_baseline(&text);
    text.vertical_align = VAlign::Bottom;
    let bottom = text_first_baseline(&text);

    assert!(top > 0.0 && top < center);
    assert!(center < bottom && bottom < text.local_size[1]);
    assert!((center - (top + bottom) * 0.5).abs() < 0.01);
}

#[test]
fn text_outline_applies_paragraph_spacing_to_later_paragraphs() {
    use fanta_doc::{TextAutoResize, TextNode};

    let mut plain = TextNode::new("A\nA", 200.0, 200.0);
    plain.auto_resize = TextAutoResize::Height;
    plain.style.size_px = 32.0;
    let mut spaced = plain.clone();
    spaced.paragraph_spacing = 24.0;

    let plain_bounds = text_node_outline(&plain)
        .and_then(|path| path.rough_bounds())
        .expect("plain text has an outline");
    let spaced_bounds = text_node_outline(&spaced)
        .and_then(|path| path.rough_bounds())
        .expect("spaced text has an outline");

    assert!((spaced_bounds.min_y - plain_bounds.min_y).abs() < 0.01);
    assert!((spaced_bounds.max_y - plain_bounds.max_y - 24.0).abs() < 0.01);
}

#[test]
fn fixed_text_outline_is_clipped_to_the_painted_box() {
    use fanta_doc::{TextAutoResize, TextNode};

    let mut text = TextNode::new("MMMM", 48.0, 18.0);
    text.style.size_px = 48.0;
    text.auto_resize = TextAutoResize::Height;
    let unclipped = text_node_outline(&text)
        .and_then(|path| path.rough_bounds())
        .expect("unclipped text has an outline");
    assert!(unclipped.max_y > text.local_size[1]);

    text.auto_resize = TextAutoResize::None;
    let clipped = text_node_outline(&text)
        .and_then(|path| path.rough_bounds())
        .expect("the text intersects its fixed box");
    assert!(clipped.min_x >= -0.01 && clipped.min_y >= -0.01);
    assert!(clipped.max_x <= text.local_size[0] + 0.01);
    assert!(clipped.max_y <= text.local_size[1] + 0.01);
}
