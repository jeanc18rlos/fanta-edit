...
/// The frame box a group's background/border occupies: its `clip_size` when set
/// (a Figma FRAME/SECTION), else the scene-computed content bounds when the
/// group still carries a background, background fills, or a border — and `None`
/// for a plain unclipped group with nothing to paint. Shared by the background
/// fill, the foreground border, and the effects silhouette so all three agree
/// on exactly which nodes have a box.
pub(crate) fn frame_box_bounds(
    g: &GroupNode,
    scene_id: Option<NodeId>,
    scene: &Scene,
) -> Option<Bounds> {
    match g.clip_size.or(g.local_size) {
...
///
/// `path_cache_id` is `Some` only for a live-scene node whose variable/motion
/// overlay did NOT replace it this frame — the id keys the cross-frame
/// [`PathCache`](super::PathCache) for vector geometry. Transient clones and
/// overlaid nodes pass `None` and build their Skia paths directly.
pub(crate) fn paint_node_content(
    ...
    match &node.data {
        ...
        NodeData::Video(v) => {
            ...
            ctx.layer_volatile = true;
            paint_video(canvas, v, scene_id, ctx);
            ContentPaintState::default()
        }
        NodeData::Audio(a) => {
            ctx.layer_volatile = true;
        ...
        NodeData::Boolean(b) => {
            // The node's shape is the fold of its operand children; paint it with
            // the node's own fills/strokes. Children are NOT recursed into by the
            // walk (they are operands, consumed here).
            super::paint_boolean(canvas, b, scene_id, ctx);
            ContentPaintState::default()
        }
        NodeData::Embed(e) => {
...
/// Paint a video node: the live filmstrip frame if playing, else the poster,
/// else the fallback card — on the shared media shadow/border, with a progress
/// bar while playing.
fn paint_video(canvas: &Canvas, v: &VideoNode, scene_id: Option<NodeId>, ctx: &mut RenderCtx) {
...
/// Paint an audio node: a real waveform from the source PCM when reachable,
/// else the placeholder — on the shared media shadow/border.
fn paint_audio(canvas: &Canvas, a: &AudioNode, scene_id: Option<NodeId>, ctx: &mut RenderCtx) {
    ...
    let drawn = ctx
        ...
        .is_some_and(|bytes| {
            super::media::draw_audio_waveform(canvas, a.local_size, &bytes, accent, progress, dark)
        });
    if drawn {
...
/// Paint a 3D node's placeholder studio card (a cube on a contact shadow).
fn paint_model3d(canvas: &Canvas, m: &Model3dNode, ctx: &mut RenderCtx) {
...
pub(crate) fn paint_node_foreground(
    ...
    if let Some(bounds) = frame_box_bounds(g, scene_id, ctx.scene) {
        stroke_box_path(
            canvas,
