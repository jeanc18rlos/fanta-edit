...
/// **logical** screen pixels; the renderer scales to physical separately.
pub fn handle_screen_position(
    ...
    world_to_screen(handle.handle_world(world_bounds), viewport, screen_size)
}
...
///
/// Closest-wins (rather than first-match) so overlapping handles on small
/// selections never produce surprises.
pub fn hit_test_resize_handle(
    ...
    let mut best: Option<(ResizeHandle, f64)> = None;
    for &h in &ResizeHandle::ALL {
        let pos = handle_screen_position(h, world_bounds, viewport, screen_size);
        let d = (pos - screen_point).length();
...
pub fn hit_test_rotate_handle(
    ...
    let mut best: Option<(RotateHandle, f64)> = None;
    for &h in &RotateHandle::ALL {
        let pos = rotate_handle_screen_position(h, world_bounds, viewport, screen_size);
        let d = (pos - screen_point).length();
        if d <= threshold && best.is_none_or(|(_, prev)| d < prev) {
            best = Some((h, d));
        }
...
pub fn hit_test_resize_handle_oriented(
    ...
    let mut best: Option<(ResizeHandle, f64)> = None;
    for &h in &ResizeHandle::ALL {
        let pos = handle_screen_position_oriented(h, local, world_t, viewport, screen_size);
        let d = (pos - screen_point).length();
        if d <= threshold && best.is_none_or(|(_, prev)| d < prev) {
            best = Some((h, d));
        }
...
pub fn hit_test_rotate_handle_oriented(
    ...
    let mut best: Option<(RotateHandle, f64)> = None;
    for &h in &RotateHandle::ALL {
        let pos = rotate_handle_screen_position_oriented(h, local, world_t, viewport, screen_size);
        let d = (pos - screen_point).length();
        if d <= threshold && best.is_none_or(|(_, prev)| d < prev) {
            best = Some((h, d));
        }
...
/// transform (rotation preserved, the linear part stays a pure rotation with NO
/// scale) and the new box size `[w, h]` the caller writes into the node's
/// `local_size`. The dragged handle's world anchor is pinned exactly as in
/// [`resize_transform_keep_rotation`]; the only difference is that the new extent
/// flows into the box size rather than the transform's scale, so glyphs keep
/// their font size while the box grows/shrinks.
pub fn resize_box_keep_rotation(
    ...
    // Resize in the un-rotated frame (identical setup to the scale path), so the
    // dragged-edge math is axis-aligned.
    let frame = original.then(&r_inv);
    let frame_bounds = local.transformed(&frame);
    let cursor_frame = r_inv.transform_point(cursor_world);
    let mut new_frame_bounds =
    ...
    // Clamp to a minimum extent on the fixed (anchor) edge, mirroring the scale
    // path so a text box can never collapse to zero/negative.
    let min_size = 0.5;
    if new_frame_bounds.width().abs() < min_size {
        if new_frame_bounds.max_x >= new_frame_bounds.min_x {
            new_frame_bounds.max_x = new_frame_bounds.min_x + min_size;
    ...
    // Map the node's local origin to the new box top-left with NO scale (the
    // whole point — the glyphs are not stretched), then rotate back by theta.
    let tx = new_frame_bounds.min_x - local.min_x;
    let ty = new_frame_bounds.min_y - local.min_y;
    let new_frame = Transform2D::from_components([1.0, 0.0, 0.0, 1.0, tx, ty]);
    (
        new_frame.then(&r),
        new_frame_bounds.width().abs(),
        new_frame_bounds.height().abs(),
    )
}
