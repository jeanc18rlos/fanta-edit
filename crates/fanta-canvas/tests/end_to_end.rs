...
/// Measurement helpers (for annotations, Figma inspect/measurements fidelity).
#[test]
fn measurement_helpers_cover_point_rect_and_bounds_cases() {
    use fanta_canvas::{
        bounds_distance, horizontal_distance, point_distance, rect_gap, vertical_distance,
    };
    use glam::DVec2;

    // point
    assert!((point_distance(DVec2::new(0., 0.), DVec2::new(3., 4.)) - 5.0).abs() < 1e-9);

    // axis
    assert!((horizontal_distance(DVec2::new(10., 5.), DVec2::new(20., 5.)) - 10.0).abs() < 1e-9);
    assert!((vertical_distance(DVec2::new(0., 0.), DVec2::new(0., 7.)) - 7.0).abs() < 1e-9);

    // rect_gap: separated on x
    let g = rect_gap(
        DVec2::new(0., 0.),
        [10., 10.],
        DVec2::new(20., 0.),
    ...
    assert!((g2[0] - 0.0).abs() < 1e-9 && (g2[1] - 0.0).abs() < 1e-9);

    // bounds_distance
    let d = bounds_distance(
        DVec2::new(0., 0.),
        DVec2::new(10., 10.),
        DVec2::new(20., 0.),
        DVec2::new(30., 10.),
