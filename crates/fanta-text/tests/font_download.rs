...
//!
//! The task requires the *online path* to genuinely fetch — so this test really
//! does hit `raw.githubusercontent.com`. But CI and offline machines must not
//! fail: a single reachability probe up front decides whether to run the live
//! assertions or cleanly no-op with a printed notice. When online it *proves*
//! the download; when offline it passes without touching the network further.
//!
//! Run it explicitly with output:
//! `cargo test -p fanta-text --test font_download -- --nocapture`
