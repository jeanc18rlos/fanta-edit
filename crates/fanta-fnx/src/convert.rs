...
pub fn tree_from_nodes(nodes: &[Value]) -> Result<FnxTree, FnxError> {
    ...
    for n in nodes {
        let id = node_id(n)?;
        if by_id.insert(id, n).is_some() {
            return Err(FnxError::Parse(format!("duplicate node id {id}")));
        }
    }
    ...
    let root = build(root_node, &kids, &mut sidecar, None)?;
    Ok(FnxTree {
        root,
        sidecar,
        root_parent,
...
fn build(
    ...
    let position = u32::try_from(sidecar.len()).ok();
    ...
    if let Some(children) = kids.get(id) {
        ...
        for child in sorted {
            el.children.push(build(child, kids, sidecar, position)?);
        }
    }
    Ok(el)
}
