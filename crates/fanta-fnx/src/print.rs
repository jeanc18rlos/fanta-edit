...
const GENERATED_HEADER: &str = "// @generated fanta source — the design is the source of truth; ids live in the .ids sidecar\n";
...
pub fn print_doc(name: &str, root: &FnxElement) -> String {
    // Sugar a clone so the caller's tree is untouched: a pure-translation
    // `transform` prints as readable `x`/`y` (see [`crate::sugar`]).
...
/// value uses a JSON-ish grammar with one shorthand: a Color object renders as a
/// `#RRGGBB[AA]` literal. Deterministic — sorted object keys, canonical scalars.
...
/// Recursively render an attribute value: Color objects become `#hex`, other
/// objects/arrays recurse, scalars/strings go through `serde_json`.
