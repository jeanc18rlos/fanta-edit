...
fn auto_layout_of<T: LayoutTree>(tree: &T, id: NodeId) -> Option<AutoLayout> {
    ...
    al.min_size = al.min_size.map(|limit| {
        limit
            .filter(|value| value.is_finite())
            .map(|value| value.max(0.0))
    });
    al.max_size = al.max_size.map(|limit| {
        limit
            .filter(|value| value.is_finite())
            .map(|value| value.max(0.0))
    });
    for (minimum, maximum) in al.min_size.into_iter().zip(al.max_size.iter_mut()) {
        if let (Some(minimum), Some(current_maximum)) = (minimum, *maximum)
            && current_maximum < minimum
        {
            // A malformed/imported constraint pair must still have deterministic
            // CSS-like semantics: the minimum wins over a smaller maximum.
...
/// Axis-aligned bounds of the child's local box after its preserved linear
/// transform, before translation. Auto-layout aligns the visible bounds, not
/// the transform pivot: a rotated vector or an offset path can have a transformed
/// minimum far from `M * local_origin`.
fn transformed_local_box(info: &ChildInfo) -> LocalBox {
    ...
    for point in points {
        let transformed = info.matrix.mul_vec2(point);
        min = min.min(transformed);
        max = max.max(transformed);
    }
    LocalBox {
        origin: [min.x, min.y],
        size: [(max.x - min.x).max(0.0), (max.y - min.y).max(0.0)],
    }
}
...
fn run_metrics(infos: &[ChildInfo], run: &[usize], horizontal: bool, spacing: f64) -> RunMetrics {
    let content_main: f64 = run
        .iter()
        .map(|&i| main_of(transformed_local_box(&infos[i]).size, horizontal))
        .sum();
...
fn run_max_cross(infos: &[ChildInfo], run: &[usize], horizontal: bool) -> f64 {
    run.iter()
        .map(|&i| cross_of(transformed_local_box(&infos[i]).size, horizontal))
...
fn distribute_grow_on_run(
    ...
    let fixed_used: f64 = run
        ...
        .map(|&i| main_of(transformed_local_box(&infos[i]).size, flow.horizontal))
        .sum();
...
fn primary_cursor(
    ...
    match al.primary_align {
        ...
        // Equal gaps INCLUDING before the first and after the last child, so
        // free space splits into `count + 1` gaps. A single child lands
        // centered (one gap each side), matching Figma.
        PrimaryAlign::SpaceEvenly => {
            let free = (align_main - metrics.content_main).max(0.0);
            let gap = free / (count + 1) as f64;
            (flow.pad_main_lo + gap, gap)
        }
    }
}
...
fn place_run(
    ...
    for &i in run {
        let transformed_size = transformed_local_box(&infos[i]).size;
        let child_cross = cross_of(transformed_size, flow.horizontal);
        let cross_pos =
        ...
        cursor += main_of(transformed_size, flow.horizontal) + gap;
    }
}
...
fn break_wrap_lines(
    ...
    for &i in flow_indices {
        let child_main = main_of(transformed_local_box(&infos[i]).size, flow.horizontal);
        if cur.is_empty() {
            cur.push(i);
            cur_main = child_main;
            continue;
...
/// Set an auto-layout container's own size without changing whether it clips.
...
/// Compute the child's transform so its transformed axis-aligned bounds land at
/// `target` in the frame's local space, preserving the child's complete linear
/// part. Using the transformed minimum rather than `M * local_origin` is what
/// keeps rotated/scaled children and vectors with offset path origins aligned by
/// their visible bounds instead of by their pivot.
fn place_child(info: &mut ChildInfo, target: DVec2) {
    let transformed = transformed_local_box(info);
    let transformed_min = DVec2::new(transformed.origin[0], transformed.origin[1]);
    info.computed_translation = Some(target - transformed_min);
}
...
fn counter_offset(align: CounterAlign, extent: f64, child_cross: f64) -> f64 {
    let free_space = (extent - child_cross).max(0.0);
    match align {
        CounterAlign::Start | CounterAlign::Stretch => 0.0,
        CounterAlign::Center => free_space * 0.5,
        CounterAlign::End | CounterAlign::Baseline => free_space,
    }
}
