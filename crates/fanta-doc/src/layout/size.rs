...
/// sized plain group its non-clipping `local_size`; a
/// vector its path's rough bounds; the box-shaped variants their `local_size`.
/// A group with neither box reports a zero box — the solver only flows
...
pub(crate) fn local_box(node: &CanvasNode) -> LocalBox {
    match &node.data {
        NodeData::Group(g) => match g.clip_size.or(g.local_size) {
            Some([w, h]) => LocalBox {
                origin: [0.0, 0.0],
                size: [w, h],
            },
...
pub(crate) fn set_size(node: &mut CanvasNode, w: f64, h: f64) {
    match &mut node.data {
        NodeData::Group(g) => {
            // Preserve whether this is a clipping frame or a sized plain group.
            if g.clip_size.is_some() {
                g.clip_size = Some([w, h]);
            } else if g.local_size.is_some() {
