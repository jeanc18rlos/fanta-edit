...
use crate::serde_util::{is_false, is_zero_f32};
...
pub enum PrimaryAlign {
    ...
    End,
    /// Distribute free space between children (CSS `space-between`): no space
    /// before the first or after the last child, equal gaps between.
    SpaceBetween,
    /// Distribute free space evenly, INCLUDING before the first and after the
    /// last child (CSS `space-evenly`): `count + 1` equal gaps.
    SpaceEvenly,
}
...
pub struct AutoLayout {
    ...
    #[serde(default)]
    pub reverse_z: bool,
    /// Minimum frame size `[width, height]` in logical px (Figma `minWidth` /
    /// `minHeight`). `None` on an axis ⇒ no lower bound. Clamps a hug axis so a
    /// hug frame never shrinks below its minimum (the common "min-width button"
    /// case). Skipped when both are `None`, so old docs round-trip identical.
    #[serde(default, skip_serializing_if = "no_size_limit")]
    pub min_size: [Option<f64>; 2],
    /// Maximum frame size `[width, height]` (Figma `maxWidth` / `maxHeight`).
    /// `None` ⇒ no upper bound. Clamps a hug axis from above.
    #[serde(default, skip_serializing_if = "no_size_limit")]
...
/// `true` when neither axis has a size limit — the common case, skipped from
/// serialization so old docs round-trip byte-identical.
fn no_size_limit(s: &[Option<f64>; 2]) -> bool {
