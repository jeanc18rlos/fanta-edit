...
impl Scene {
    ...
    /// The local AABB of a node in its own coordinate space. A clipping group
    /// reports its clip box; a non-clipping group unions its explicit box (when
    /// present) with every transformed descendant bound.
    ///
    ...
    fn compute_local_bounds(&self, id: NodeId) -> Option<Bounds> {
        ...
        if let NodeData::Group(group) = &node.data {
            let clips_children = group.clip_size.is_some()
                && node
                    ...
                    .unwrap_or(true);
            if clips_children && let Some([width, height]) = group.clip_size {
                return Some(Bounds::from_xywh(0.0, 0.0, width, height));
            }
            ...
            for &child_id in self.children_of(Some(id)) {
                let Some(child_local) = self.local_bounds(child_id) else {
                    continue;
                };
                let Some(child_node) = self.nodes.get(&child_id) else {
                    continue;
                };
                let Some(in_group_space) = child_local.try_transformed(&child_node.transform)
                else {
                    continue;
                };
                bounds = Some(match bounds {
                    Some(existing) => existing.union(&in_group_space),
                    None => in_group_space,
                });
            }
            return bounds;
        ...
        // A boolean-op node has no intrinsic bounds: union its operands,
        // transformed into our local space. This is a superset of the true
        // folded bounds (a Subtract/Intersect shrinks it), which is a safe
        // over-approximation for culling / AABB hit-testing without path ops in
        // this Skia-free crate. A pathless vector also reaches here and unions
        // nothing → `None`.
        if node.data.as_boolean().is_none() {
            return None;
        }
        ...
        for &child_id in self.children_of(Some(id)) {
            if let Some(child_local) = self.local_bounds(child_id) {
                ...
                if let Some(in_self_space) = child_local.try_transformed(&child_node.transform) {
                    acc = Some(match acc {
