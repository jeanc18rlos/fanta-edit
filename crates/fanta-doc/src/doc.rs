//! [`Doc`] — the top-level Fantaisa document.
//!
...
/// the `PathData` shape. See `fanta-format::migrate` for the v1→v2 step.
...
pub struct Doc {
    ...
    /// here; `None` falls back to the active page.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl Doc {
    ...
    /// library + variable registry + mode map + flow start — built here by
    /// disjoint field borrows so it doesn't conflict with the `&mut history`.
