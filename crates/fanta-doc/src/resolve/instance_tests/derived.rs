...
#[test]
fn expand_applies_derived_size_to_non_clipping_group_box() {
    use crate::node::DerivedOverride;

    let mut scene = Scene::new();
    let (library, component, root_id, _) = master_with_vector(&mut scene);
    let mut loose_group = CanvasNode::new(NodeData::Group(GroupNode::default()));
    loose_group.parent = Some(root_id);
    let loose_group_id = loose_group.id;
    scene.insert(loose_group).unwrap();
    let instance = InstanceNode {
        component,
        overrides: Vec::new(),
        prop_values: Default::default(),
        derived: vec![DerivedOverride {
            path: smallvec![loose_group_id],
            transform: None,
            size: Some([70.0, 35.0]),
            fills: None,
            path_data: None,
    ...
    let group = expanded
        .iter()
        .find(|entry| entry.def_path.as_slice() == [loose_group_id])
        .expect("derived group clone");
    let NodeData::Group(group) = &group.node.data else {
        panic!("expected group");
    };
    assert_eq!(group.local_size, Some([70.0, 35.0]));
...
#[test]
fn explicit_overrides_win_over_baked_derived_text_on_the_same_clone() {
    // The user's live edit to instance text is a sparse `TextContent` /
    // `FillColor` override; the derived entry is Figma's import-time snapshot
    // of the SAME channels. Without the claims guard the derived write (which
    // runs after the overrides) silently reverts the edit on every
    // re-expansion — the edit is invisible on canvas while the editor overlay
    // shows the new text.
    use crate::Fill;
    use crate::node::{DerivedOverride, DerivedText};
    let mut scene = Scene::new();
    let (lib, comp_id, _root_id, label_id) = master(&mut scene);
    let baked = Color::rgb(0xEE, 0xEE, 0xEE);
    let edited_color = Color::rgb(0xFF, 0x00, 0x00);
    let inst = InstanceNode {
        component: comp_id,
        overrides: vec![
            Override {
            ...
            Override {
                ...
                value: OverrideValue::Fills {
                    fills: smallvec![Fill::solid(edited_color)],
                },
            },
        ],
        prop_values: Default::default(),
    ...
    match &child.node.data {
        NodeData::Text(t) => {
            assert_eq!(t.content, "Edited", "content override must survive derived");
            assert_eq!(
                t.style.color, edited_color,
                "fill override must survive the baked theme color"
            );
            // Unclaimed channels still come from the derived snapshot.
            assert_eq!(t.style.size_px, 24.0, "derived font size still applies");
            assert_eq!(t.style.weight, 600, "derived weight still applies");
        }
        other => panic!("expected text, got {other:?}"),
    }
}
...
#[test]
fn derived_text_without_matching_override_still_applies_fully() {
    // The guard is per-path: an override on a DIFFERENT descendant must not
    // suppress the derived write on this one.
    use crate::node::{DerivedOverride, DerivedText};
    let mut scene = Scene::new();
    let (lib, comp_id, _root_id, label_id) = master(&mut scene);
    let baked = Color::rgb(0xEE, 0xEE, 0xEE);
    let inst = InstanceNode {
        component: comp_id,
        overrides: vec![Override {
            // Unrelated path: a random id that is not the label clone.
            target_path: smallvec![NodeId::new()],
            target_prop: BoundProp::TextContent,
            value: OverrideValue::Text {
    ...
    match &child.node.data {
        NodeData::Text(t) => {
            assert_eq!(t.content, "Baked");
            assert_eq!(t.style.color, baked);
        }
        other => panic!("expected text, got {other:?}"),
    }
}
